			...
			BEGIN SEQUENCE
				...
				If ojson["items"][_nni]["state"] == "SUBMITTED"
					...
					Else
						...
						_cTrIdJson	:= ""
						_cPaymMeth	:=	""
						_cGatePg	:=	alltrim(ojson["items"][_nni]["paymentGroups"][1]["gatewayName"])
						if valtype(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["transactionId"]) == "C"

							If ! _cGatePg == "NewInvoiceGateway"

								If valtype(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["type"]) == "C"
									If "PIX" $ ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["type"]
										_cPaymMeth := "PIXJPAYMEN"
									Endif
								Endif

								If _cGatePg	==	"getnetGateway"
									_cnsu := alltrim(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["acquirer_transaction_id"])
									_cNomtag	:=	"payment_id"
								ElseIf _cGatePg	==	"payu-latam"
									_cRawRes	:=	alltrim(ojson["items"][_nni]["paymentGroups"][1]["paymentProps"]["rawResponse"])
								...
								ElseIf "jpayment-" $_cGatePg
									_cRawRes	:=	alltrim(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["rawResponse"])
									_cRawRes	:=	Decode64(_cRawRes)
									oJsonRaw	:= JsonObject():new()
									fwJsonDeserialize( _cRawRes, @oJsonRaw )
								...
								_cpayid := alltrim(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"][_cNomtag])
								_cidprof := alltrim(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["card_id"])

								If "jpayment-"	$	_cGatePg
									If ojson["items"][_nni]["paymentGroups"][1]["amountAuthorized"]	<> Nil
										_nvalres := ojson["items"][_nni]["paymentGroups"][1]["amountAuthorized"] * _nTxCaucao
									Endif
								Else
									If ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["valorReservado"]	<> Nil
										_nvalres := VAL(ojson["items"][_nni]["paymentGroups"][1]["authorizationStatus"][1]["statusProps"]["valorReservado"])
									Endif
								Endif
							Endif
						Endif
						...
						ZC5->ZC5_HRRECE  := TIME()
						ZC5->ZC5_IDSFA  := alltrim(ojson["items"][_nni]["id"])
						ZC5->ZC5_INTEGR	:= "P"
