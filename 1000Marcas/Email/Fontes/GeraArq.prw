...
GeraArq - Alteração de dados contas a pagar
@author Wagner Neves
@since 17/06/2024
@version 1.0
@type function
/*/
User Function GeraArq(cDoc, cSerie)
	Local cPrefix   := ""
	Local cNum      := ""
	Local cParc     := ""
	Local cTipo     := ""
	...
	//Busca Numero do titulo
	cQuery := " SELECT DISTINCT E1_PREFIXO, E1_NUM, E1_PARCELA, E1_TIPO, E1_NUMNOTA, E1_SERIE "
	cQuery += " FROM " + RetSQLname("SE1") + " SE1 "
	cQuery += " WHERE E1_NUMNOTA = '"+cDoc+"' AND E1_SERIE = '"+cSerie+"' "
	...
	If !(cAlias)->(EOF())
		...
		cNum    := (cAlias)->E1_NUM
		cParc   := (cAlias)->E1_PARCELA
		cTipo   := (cAlias)->E1_TIPO

		//Gera Boleto Itau
		// GerBolItau(cPrefix, cNum, cParc, cTipo )

		//Gera Danfe
		GerDanfe(cDoc, cSerie)
	EndIf

Return

Static Function GerBolItau(_cPrefix, _cNum, _cParcela, _cTipo )
	...
	SE1->(DbGoTop())
	If SE1->(DbSeek( xFilial("SE1")+ _cPrefix + _cNum + _cParcela + _cTipo))
...
static function FGERANNU(cNoss)
	local nCont 	:= 0
	local nPeso 	:= 1
	local cRet		:= ''
	local nResto	:= 0
...
Static Function GerDanfe(_cNota,_cSerie)//u_109IDANFE('000003268','2')
	Local aArea     := GetArea()
	Local cPasta	:= ""
	Local cIdent    := ""
	Local oDanfe    := Nil
