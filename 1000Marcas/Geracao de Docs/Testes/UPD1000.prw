...
/*/
//--------------------------------------------------------------------
User Function XUPD1000( cEmpAmb, cFilAmb )
	Local   aSay      := {}
	Local   aButton   := {}
	Local   aMarcadas := {}
	...
	If lOk
		...
		If !Empty( aMarcadas )
			If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
				...
				If lAuto
					If lOk
						MsgInfo( "Atualização realizada.", "UPD1000" )
					Else
...
//
	aAdd( aSX3, { ;
		{ 'AI0'																	, .T. }, ; //X3_ARQUIVO
	{ '23'																	, .T. }, ; //X3_ORDEM
	{ 'AI0_XRECCB'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Recebe Cobr?'														, .T. }, ; //X3_TITULO
	{ 'Recebe Cobr?'														, .T. }, ; //X3_TITSPA
	{ 'Recebe Cobr?'														, .T. }, ; //X3_TITENG
	{ 'Recebe Cobrança ?'													, .T. }, ; //X3_DESCRIC
	{ 'Recebe Cobrança ?'													, .T. }, ; //X3_DESCSPA
	{ 'Recebe Cobrança ?'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	aAdd( aSX3, { ;
		{ 'SF2'																	, .T. }, ; //X3_ARQUIVO
	{ '227'																	, .T. }, ; //X3_ORDEM
	{ 'F2_XENMAIL'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Doc Env Mail'														, .T. }, ; //X3_TITULO
	{ 'Doc Env Mail'														, .T. }, ; //X3_TITSPA
	{ 'Doc Env Mail'														, .T. }, ; //X3_TITENG
	{ 'Docs Enviados por Email'												, .T. }, ; //X3_DESCRIC
	{ 'Docs Enviados por Email'												, .T. }, ; //X3_DESCSPA
	{ 'Docs Enviados por Email'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ '"2"'																	, .T. }, ; //X3_RELACAO
...
Static Function FSAtuSX6()
	...
	'Formato anomesdia exemplo: 20240701'									, ; //X6_DSCSPA
	''																		, ; //X6_DSCENG
	''																		, ; //X6_DESC1
	''																		, ; //X6_DSCSPA1
	''																		, ; //X6_DSCENG1
...
//
	aHlpPor := {}
	aAdd( aHlpPor, 'Recebe Cobrança ?' )
	aAdd( aHlpPor, '1=Sim' )
	aAdd( aHlpPor, '2=Nâo' )

	PutSX1Help( "PAI0_XRECCB", aHlpPor, {}, {}, .T.,,.T. )
	AutoGrLog( "Atualizado o Help do campo " + "AI0_XRECCB" )
