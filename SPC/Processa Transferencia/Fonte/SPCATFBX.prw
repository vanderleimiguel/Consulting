...
/*/{Protheus.doc} SPCATFBX
Funcao para processamento de baixa e contabilizacao de ativos
@author Wagner Neves
@since 21/06/2024
@version 1.0
@type function
/*/
User Function SPCATFBX()
	Private oDlg
...
Static Function fMontaTela()
	...
	DEFINE DIALOG oDlg TITLE "Importação, Baixa e Contabilizacao Ativo Fixo" FROM 180,180 TO 230,345 // Usando o método New
...
Static Function ProcAtivo()
	...
	//Contabilizar
	If nProcOk > 0
		If msgYesNo("Deseja contabilizar as baixas?","Contabilização") 
			fContaAux()
		EndIf
	EndIf
...
Static Function fContabil(_nTotZW2)
	...
	If !(cAliasBx)->(EOF())
		...
		cLote			:= "008860"
		cSbLote 		:= "001"
		cDoc    		:= fDocNum((cAliasBx)->ZW2_FILIAL, cLote, cSbLote)
		cOrigem         := "SPCATFBX - " + DTOC(dDataBase) + " - " + Time() + " - " + UsrRetName(cCodUser) 
		aItens 			:= {}
		aCab 			:= {}
		cLinha     		:= "001"
		...
		For nI := 1 To Len(aGrupos)
			...
			//Busca dados da Baixa
			cQryBx := "SELECT ZW2_FILIAL, ZW2_TIPO, ZW2_STATUS, ZW2_DEBITO, ZW2_CREDIT, ZW2_VALOR, ZW2_GRUPO, ZW2_ESPECI "
			cQryBx += "FROM "+RetSqlName("ZW2")+" ZW2 "
			cQryBx += "WHERE ZW2_STATUS='2' AND ZW2_TIPO = '1' AND ZW2_FILIAL = '"+xFilial("ZW2")+"' "
	...
	If _nTotZW2 > 0
		...
		ElseIf nContErr > 0 .AND. nContOK = 0
			MsgInfo("Não foram contabilizados nenhum movimentos.","Contabiliza Ativos")
		EndIf
	Else
