				...
				If cBaixaAtu = "0"
					...
					Else
						...
						If SNG->(DBSeek(FWXFilial('SNG') + cGrpDest))
							...
							cCDepre := SNG->NG_CDEPREC
							cCCdepr := SNG->NG_CCDEPR
							cCCusto := SNG->NG_CCUSTO
						...
						SN1->N1_GRUPO 	:= cGrpDest
						SN1->N1_STATUS  := "1"
						SN1->N1_INIAVP  := dAquisic
						SN1->N1_VLAQUIS := nValOri
						SN1->N1_TPAVP   := "1"
						SN1->N1_CALCPIS := "2"
						SN1->N1_PENHORA := "0"
						...
						SN3->N3_FILIAL	:= cDesFil 	
						SN3->N3_CBASE 	:= cBase 	
						SN3->N3_ITEM 	:= cItem 	
						SN3->N3_TIPO 	:= cTipo 	
						SN3->N3_BAIXA 	:= "0" 		
						SN3->N3_HISTOR	:= cDescric 
						SN3->N3_CCONTAB	:= cContab 	
						SN3->N3_DINDEPR	:= dIndDepr	
						SN3->N3_VORIG1	:= nValOri 	
						SN3->N3_TXDEPR1	:= nTaxa1 	
						SN3->N3_VRDBAL1	:= nDprBal 	
						SN3->N3_VRDMES1	:= nDprMes 	
						SN3->N3_VRDACM1	:= nDprAcum 
						SN3->N3_AQUISIC := dAquisic
						SN3->N3_TPSALDO := "1"
						SN3->N3_TPDEPR  := "1"
						SN3->N3_CDEPREC := cCDepre
						SN3->N3_CCDEPR  := cCCdepr
						SN3->N3_ATFCPR  := "2"
						SN3->N3_INTP    := "2"
						SN3->N3_CUSTBEM := cCCusto
						SN3->N3_TXDEPR2	:= nTaxa1 
						SN3->N3_TXDEPR3	:= nTaxa1 
						SN3->N3_TXDEPR4	:= nTaxa1 
						SN3->N3_TXDEPR5	:= nTaxa1 
						SN3->N3_CALCDEP	:= "0"
						SN3->N3_SEQ		:= "001"
						SN3->N3_FILORIG := cDesFil
						SN3->N3_RATEIO  := "2"

						SN3->(MsUnLock())

						cQuery := "SELECT MAX(N4_IDMOV) AS MAXID "
						cQuery += "FROM "+RetSqlName("SN4")+" SN4 "
						cQuery += "WHERE SN4.D_E_L_E_T_ = '' "
						DbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),cAliasSN4,.F.,.T.)
						(cAliasSN4)->(DbGoTop())
						cIdSN4 := SOMA1((cAliasSN4)->MAXID)
						
						DbSelectArea("SN4")
						RecLock("SN4", .T.)	

						SN4->N4_FILIAL	:= cDesFil 	
						SN4->N4_CBASE 	:= cBase 	
						SN4->N4_ITEM 	:= cItem 	
						SN4->N4_TIPO 	:= cTipo 	
						SN4->N4_OCORR 	:= "05"
						SN4->N4_TIPOCNT	:= "1" 
						SN4->N4_CONTA	:= cContab 	
						SN4->N4_DATA	:= dAquisic	
						SN4->N4_QUANTD	:= 0	
						SN4->N4_VLROC1	:= nValOri	
						SN4->N4_TXMEDIA := 0
						SN4->N4_TXDEPR  := nTaxa1
	...
	While nSeq <= 2
		...
		ElseIf nSeq = 2
			...
			For nI := 1 To Len(aGrupos)
				...
				CT2->CT2_MOEDLC		:= "01" 						
				CT2->CT2_DC 		:= "3"						
				CT2->CT2_DEBITO		:= cCT2Debi					
				CT2->CT2_CREDIT		:= cCT2Cred 				
				CT2->CT2_VALOR 		:= nCT2Val				 	
				CT2->CT2_ORIGEM		:= cOrigem					
				CT2->CT2_HIST 		:= cCT2Hist 
				CT2->CT2_ROTINA     := "SPCTRFAF"
		...
		//Execauto se estiver Contabilização Origem
		If nSeq = 1
			//Execauto
			lMsErroAuto	:= .F.
			Begin Transaction
