...
/*/
//-----------------------------------------------------------------------
// Customizações nas linhas: 173,443,779,1400,2333,2791
//-----------------------------------------------------------------------
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
					...
					If lLJPRF
						BeginSql Alias cAliasSE1
							...
							SELECT 	E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_ORIGEM,E1_CSLL,E1_COFINS,E1_PIS,E1_PIS,E1_IRRF,E1_INSS,E1_ISS,E1_MOEDA,E1_CLIENTE,E1_LOJA,
									R_E_C_N_O_ AS E1_RECNO
							FROM %Table:SE1% SE1
							WHERE
							SE1.E1_FILIAL = %xFilial:SE1% AND
							//-----------------------------------------------------------------------------------------|
							// Tratamento da filial de origem para posicionamento correto dos títulos quanto á filial  |
							// pois a TOTVS não trata na query padrão tabela compartilhada. 						   |
							// Raphael Koury Giusti - Data: 30/12/2016.												   |
							//-----------------------------------------------------------------------------------------| 
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.SF2->F2_DUPL == (cAliasSE1)->E1_NUM .AND.;
					...
					If lNfsePcc
						If Alltrim((cAliasSE1)->E1_TIPO) $ "NF"
							SE1->(dbGoTo((cAliasSE1)->E1_RECNO))		
							/* Alterado este codigo para utilizacao do ajuste abaixo
							nRetCsl += (cAliasSE1)->E1_CSLL 
							nRetCof	+= (cAliasSE1)->E1_COFINS
							nRetPis += (cAliasSE1)->E1_PIS
							*/
							/*--------------------------------------------------------------------------\		
							| DTM | Fabio Hayama                                    27/03/2019  		|		
							| Autor: Fabio Hayama                                                       |		
							| Objetivo: Verificar se o cliente recolhe o ISS ou não                     |		
							\--------------------------Inicio------------------------------------------*/								
							cRecISS := "2"
							SC5->(DbSetOrder(1))
							if(SC5->(DbSeek(xFilial("SC5") + PadR(SE1->E1_PEDIDO ,TamSx3("C5_NUM")[1]))))
								cRecISS := SC5->C5_RECISS 									
							endIf
															
							if(Alltrim(cRecISS) == "1") //Recolhe ISS = SIM (1) 
								aPCC := newMinPcc(DataValida(SE1->E1_VENCTO,.T.), SE1->E1_SALDO + E1_ISS, SE1->E1_NATUREZ, "R", SA1->(A1_COD+A1_LOJA))
							else
								aPCC := newMinPcc(DataValida(SE1->E1_VENCTO,.T.), SE1->E1_SALDO			, SE1->E1_NATUREZ, "R", SA1->(A1_COD+A1_LOJA))									
							endIf
							
							If SuperGetMv('ZZ_UPDPCC3',.F.,.T.)		
								nRetCsl += aPCC[4] // CSL - Baixa		
								nRetCof += aPCC[3] // COF - Baixa		
								nRetPis += aPCC[2] // PIS - Baixa		
							Else		
								nRetCsl += (cAliasSE1)->E1_CSLL 		
								nRetCof	+= (cAliasSE1)->E1_COFINS			
								nRetPis += (cAliasSE1)->E1_PIS		
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//Adicionado Deivid Lima - 09/12/16 - Descriçao Aglutinada do LIMS via integração OTC
				if !lAglutina .and. SC5->C5_ZZTPIM == "A"
					lAglutina := .t.
				endif 
				//Fim		
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		if !empty(aNota)
			//Adicionado Leonardo Azevedo - 16/05/14 - PE atualização de mensagens BIOAGRI
			U_BIOPENFSE(@cDiscrNFSe,aDupl)
			//Fim
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf, lAglutina)
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf, lAglutina) //Customização - Fabio Hayama - Geeker Comapny - Adicionar o lAglutina	
	...
	for nX := 1 to len( aProd )
		...
		EndIf
		//-------------------------------------|
		// Tratamento para itens aglutinados   |
		// Raphael Koury Giusti - 27/01/2017   |
		//-------------------------------------|
		If !lAglutina
			cString	+= "<quant>" + allTrim( convType( aProd[nX][9], 15, 2 ) ) + "</quant>"
			cString	+= "<valunit>" + allTrim( convType( aProd[nX][10], 15, 2 ) ) + "</valunit>"
		Else 
			cString	+= "<quant>" + allTrim( convType( "1", 15, 2 ) ) + "</quant>"
			cString	+= "<valunit>" + allTrim( convType( aProd[nX][28], 15, 2 ) ) + "</valunit>"
		EndIf			
		//cString	+= "<quant>" + allTrim( convType( aProd[nX][9], 15, 2 ) ) + "</quant>"
...
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe )
	...
	// Removido a variável cMensCli pois a mesma estava carregando 
	// a descrição dos serviços estourando o tamanho do campo e duplicando
	// informação - Raphael Koury Giusti - 25/04/2018.
