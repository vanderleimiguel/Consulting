...
User Function ENV_NFSE(cEmail, cArquivXML, cArquivPDF)
	Local cHTML     := ""
	Local nX     	:= 0
	Local aEmails 	:= {}
    Local cNomeXML  := ''
    ...
    For nX := 1 To Len(aEmails)
	    lRet := SendMail(aEmails[nX],'NFS-e',cHTML,cArquivXML,cNomeXML,cArquivPDF, cNomePDF)
    NEXT

Return lRet

Static Function SendMail(cEmail,cAssunto,cHtml,cPathXML,cFiNameXML,cPathPDF,cFiNamePDF)
	Local xRet
	Local lRet := .T.
	...
	If File(cPathXML)
		If oMessage:AttachFile( cPathXML ) < 0  //"\system\file.pdf"
			lRet := .f.
		    return lRet
		Else
			//adiciona uma tag informando que é um attach e o nome do arq
			oMessage:AddAtthTag('Content-Disposition: attachment; filename='+cFiNameXML)
		EndIf
	Else
		lRet := .f.
		return lRet
    ...
    If File(cPathPDF)
		If oMessage:AttachFile( cPathPDF ) < 0  //"\system\file.pdf"
			lRet := .f.
		    return lRet
		Else
			//adiciona uma tag informando que é um attach e o nome do arq
			oMessage:AddAtthTag('Content-Disposition: attachment; filename='+cFiNamePDF)
		EndIf
	Else
		lRet := .f.
		return lRet
